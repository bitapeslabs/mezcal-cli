...
import {firstTaprootAddress, generateWallet} from '../../lib/wallet.js'
...
export default class WalletCreate extends Command {
  ...
  static override flags = {
    ...
    name: Flags.string({
      char: 'n',
      description: 'label stored inside the wallet file (optional)',
    }),
  }
  ...
  public async run(): Promise<void> {
    ...
    if (fs.existsSync(target)) {
      this.error(`You already have a wallet on your system at ${target}. Use --help to see what you can do with it :)`)
      return
    }
    ...
    const {password} = await inquirer.prompt<{password: string}>([
      {
        type: 'password',
        name: 'password',
        message: "Choose a wallet password (keep it short & sweet; you'll use it often)",
        mask: '*',
    ...
    this.log(`✓ Your Mnemonic (write this down! you will need it to recover your wallet): ${mnemonic}`)
    this.log('Keep your password safe – you will need it to unlock the wallet.\n')
  }
}
